...
public class PortfolioSellTest {
	...
	public void testSellOK() {
		...
		assertEquals(9990, testing.getMonetaryValue(), 001);
		assertEquals(message, "Capital: 9890.0"
							 +"\nStocks:" 
							 +"\nABC, 10 Shares" 
							 +"\nMonetary Value: 9990.0"
							 +"\nProfit / Loss: -10.0\n", testing.toString());
	}
	...
	public void testSellAllRemovesStock() {
		...
		assertEquals(9990, testing.getMonetaryValue(), 001);
		assertFalse(message, testing.getAllStocks().contains(stock1));
	}
	...
	public void testSellTooManyShares() {
		...
		assertEquals(message, "Capital: 9795.0"
							 +"\nStocks:" 
							 +"\nABC, 20 Shares" 
